  ...
  extends zzaa
{
  ...
  private final Thread.UncaughtExceptionHandler ama = new zzb("Thread death: Uncaught exception on worker thread");
  private final Thread.UncaughtExceptionHandler amb = new zzb("Thread death: Uncaught exception on network thread");
  ...
  private void zza(zzc<?> paramzzc)
  {
    synchronized (amc)
    {
      ...
      if (alW == null)
      {
        alW = new zzd("Measurement Worker", alY);
        alW.setUncaughtExceptionHandler(ama);
        alW.start();
        return;
      }
      alW.zznm();
  ...
  private void zza(FutureTask<?> paramFutureTask)
  {
    synchronized (amc)
    {
      alZ.add(paramFutureTask);
      if (alX == null)
      {
        alX = new zzd("Measurement Network", alZ);
        alX.setUncaughtExceptionHandler(amb);
        alX.start();
        return;
      }
      alX.zznm();
  ...
  public void zzl(Runnable paramRunnable)
    throws IllegalStateException
  {
    zzzg();
    ...
    implements Thread.UncaughtExceptionHandler
  {
    ...
    public void uncaughtException(Thread paramThread, Throwable paramThrowable)
    {
      try
      {
        zzbsz().zzbtr().zzj(amg, paramThrowable);
        return;
      }
      finally
      {
    ...
    implements Comparable<zzc>
  {
    ...
    zzc(Runnable paramRunnable, boolean paramBoolean, String paramString)
    {
      ...
      if (ami == Long.MAX_VALUE) {
        zzbsz().zzbtr().log("Tasks index overflow");
      }
    }
    ...
    public int zzb(zzc paramzzc)
    {
      ...
      zzbsz().zzbts().zzj("Two tasks share the same index. index", Long.valueOf(ami));
      return 0;
    }
  }
    ...
    extends Thread
  {
    ...
    {
      zzbsz().zzbtt().zzj(String.valueOf(getName()).concat(" was interrupted"), paramInterruptedException);
    }
