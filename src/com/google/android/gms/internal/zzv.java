  ...
  implements zzb
{
  ...
  {
    zza localzza = (zza)zzbw.get(paramString);
    if (localzza != null)
    {
      zzbx -= zzca;
      zzbw.remove(paramString);
    }
  }
  ...
  {
    if (!zzbw.containsKey(paramString)) {}
    zza localzza;
    long l;
    for (zzbx += zzca;; zzbx = (zzca - zzca + l))
    {
      zzbw.put(paramString, paramzza);
      return;
      localzza = (zza)zzbw.get(paramString);
      l = zzbx;
    }
  }
  ...
  private void zzc(int paramInt)
  {
    ...
    for (;;)
    {
      ...
      Iterator localIterator = zzbw.entrySet().iterator();
      int i = 0;
      zza localzza;
      if (localIterator.hasNext())
      {
        ...
        if (zzf(zzcb).delete())
        {
          ...
          i += 1;
          if ((float)(zzbx + paramInt) >= zzbz * 0.9F) {
            break label229;
          }
        }
      }
      for (;;)
      {
        ...
        zzs.zza("pruned %d files, %d bytes, %d ms", new Object[] { Integer.valueOf(i), Long.valueOf(zzbx - l1), Long.valueOf(SystemClock.elapsedRealtime() - l2) });
        return;
        zzs.zzb("Could not delete cache entry for key=%s, filename=%s", new Object[] { zzcb, zze(zzcb) });
        break label119;
        break;
      }
    }
  ...
  {
    return new File(zzby, zze(paramString));
  }
