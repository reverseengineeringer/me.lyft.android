  ...
  extends FrameLayout
{
  ...
  {
    super(paramContext);
    adActivity = ((TuneAdActivity)paramContext);
    close = new ImageButton(paramContext);
    ...
    close.setBackgroundDrawable((Drawable)localObject);
    ...
    close.setLayoutParams(paramContext);
    ...
    post(new Runnable()
    {
      public void run()
      {
        ...
        close.getHitRect((Rect)localObject);
        int i = (int)(12.0F * f + 0.5D);
        top -= i;
        left -= i;
        bottom += i;
        right += i;
        localObject = new TouchDelegate((Rect)localObject, close);
        if (View.class.isInstance(close.getParent())) {
          ((View)close.getParent()).setTouchDelegate((TouchDelegate)localObject);
        }
      }
    });
  }
...
/* Location:
 * Qualified Name:     com.tune.crosspromo.TuneCloseButton
