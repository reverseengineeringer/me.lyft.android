...
public enum CardType
{
  ...
  static
  {
    ...
    $VALUES = new CardType[] { AMEX, DINERSCLUB, DISCOVER, JCB, MASTERCARD, VISA, MAESTRO, UNKNOWN, INSUFFICIENT_DIGITS };
    minDigits = 1;
    ...
    Iterator localIterator = getIntervalLookup().entrySet().iterator();
    while (localIterator.hasNext())
    {
      Map.Entry localEntry = (Map.Entry)localIterator.next();
      minDigits = Math.max(minDigits, ((String)getKeyfirst).length());
      if (getKeysecond != null) {
        minDigits = Math.max(minDigits, ((String)getKeysecond).length());
      }
    }
  }
  
  private CardType(String paramString)
  {
    name = paramString;
  }
  ...
  public static CardType fromCardNumber(String paramString)
  {
    ...
    {
      Map.Entry localEntry = (Map.Entry)localIterator.next();
      if (isNumberInInterval(paramString, (String)getKeyfirst, (String)getKeysecond)) {
  ...
  public static CardType fromString(String paramString)
  {
    ...
    if (i < j)
    {
      ...
      if ((localCardType == UNKNOWN) || (localCardType == INSUFFICIENT_DIGITS)) {}
      while (!paramString.equalsIgnoreCase(localCardType.toString()))
      {
        i += 1;
        break;
      }
  ...
  private static Pair<String, String> getNewPair(String paramString1, String paramString2)
  {
  ...
  private static boolean isNumberInInterval(String paramString1, String paramString2, String paramString3)
  {
    int i = Math.min(paramString1.length(), paramString2.length());
    int j = Math.min(paramString1.length(), paramString3.length());
    if (Integer.parseInt(paramString1.substring(0, i)) < Integer.parseInt(paramString2.substring(0, i))) {}
    while (Integer.parseInt(paramString1.substring(0, j)) > Integer.parseInt(paramString3.substring(0, j))) {
      return false;
    }
    return true;
  }
  ...
  public String getDisplayName(String paramString)
  {
    ...
    default: 
      return null;
    case ???: 
      return LocalizedStrings.getString(StringKey.CARDTYPE_AMERICANEXPRESS, paramString);
    case ???: 
    case ???: 
      return LocalizedStrings.getString(StringKey.CARDTYPE_DISCOVER, paramString);
    case ???: 
      return LocalizedStrings.getString(StringKey.CARDTYPE_JCB, paramString);
    case ???: 
      return LocalizedStrings.getString(StringKey.CARDTYPE_MASTERCARD, paramString);
    }
    return LocalizedStrings.getString(StringKey.CARDTYPE_VISA, paramString);
  }
  
  public Bitmap imageBitmap(Context paramContext)
  {
    String str = null;
    switch (this)
    {
    ...
    while (str != null)
    {
      return ViewUtil.base64ToBitmap(str, paramContext);
