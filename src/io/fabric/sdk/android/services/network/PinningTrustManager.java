  ...
  implements X509TrustManager
{
  ...
  {
    systemKeyStore = paramSystemKeyStore;
    pinCreationTimeMillis = paramPinningInfoProvider.getPinCreationTimeInMillis();
    ...
    while (i < j)
    {
      ...
      pins.add(hexStringToByteArray(paramPinningInfoProvider));
      i += 1;
    }
  }
  ...
  {
    if ((pinCreationTimeMillis != -1L) && (System.currentTimeMillis() - pinCreationTimeMillis > 15552000000L))
    {
      Fabric.getLogger().w("Fabric", "Certificate pins are stale, (" + (System.currentTimeMillis() - pinCreationTimeMillis) + " millis vs " + 15552000000L + " millis) " + "falling back to system trust.");
      return;
    }
    paramArrayOfX509Certificate = CertificateChainCleaner.getCleanChain(paramArrayOfX509Certificate, systemKeyStore);
  ...
  private void checkSystemTrust(X509Certificate[] paramArrayOfX509Certificate, String paramString)
    throws CertificateException
  {
    TrustManager[] arrayOfTrustManager = systemTrustManagers;
  ...
  private TrustManager[] initializeSystemTrustManagers(SystemKeyStore paramSystemKeyStore)
  {
    try
    {
      ...
      localTrustManagerFactory.init(trustStore);
