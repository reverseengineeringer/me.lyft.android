  ...
  implements EventsStorage
{
    ...
    throws IOException
  {
    ...
    workingDirectory = paramFile;
    targetDirectoryName = paramString2;
    workingFile = new File(workingDirectory, paramString1);
  ...
  public void add(byte[] paramArrayOfByte)
    throws IOException
  {
    queueFile.add(paramArrayOfByte);
  }
  
  public boolean canWorkingFileStore(int paramInt1, int paramInt2)
  {
    return queueFile.hasSpaceFor(paramInt1, paramInt2);
  }
  ...
  public void deleteFilesInRollOverDirectory(List<File> paramList)
  {
    ...
    while (paramList.hasNext())
    {
      ...
      CommonUtils.logControlled(context, String.format("deleting sent analytics file %s", new Object[] { localFile.getName() }));
      localFile.delete();
    }
  }
  ...
  public List<File> getBatchOfFilesToSend(int paramInt)
  {
    ...
    File[] arrayOfFile = targetDirectory.listFiles();
  ...
  {
    queueFile.close();
    move(workingFile, new File(targetDirectory, paramString));
    queueFile = new QueueFile(workingFile);
  }
}
