...
{
  static final IDeferredCall EMPTY_CALL = new IDeferredCall()
  {
    public void callUsing(ICallDependencyFactory paramAnonymousICallDependencyFactory)
      throws Exception
    {}
    
    public boolean needToRetry(Exception paramAnonymousException)
    {
      return false;
    }
  ...
  PersistentQueueManager(File paramFile, ICallDependencyFactory paramICallDependencyFactory, CallFactory paramCallFactory)
  {
    persistentQueue = new PersistentQueue(paramFile, paramCallFactory);
    callDependencyFactory = paramICallDependencyFactory;
  }
  
  void add(IDeferredCall paramIDeferredCall)
  {
    persistentQueue.add(paramIDeferredCall);
  }
  
  boolean executeNext()
  {
    if (executingRef.getAndSet(true)) {
      return false;
    }
    IDeferredCall localIDeferredCall = persistentQueue.peek();
    try
    {
      localIDeferredCall.callUsing(callDependencyFactory);
      bool = persistentQueue.remove(localIDeferredCall);
      return bool;
    }
    catch (Exception localException)
    {
      boolean bool;
      if (!localIDeferredCall.needToRetry(localException))
      {
        bool = persistentQueue.remove(localIDeferredCall, localException);
        return bool;
      }
      return false;
    }
  ...
  static class PersistentQueue
  {
    ...
    PersistentQueue(File paramFile, FileObjectQueue.Converter<IDeferredCall> paramConverter)
    {
...
/* Location:
 * Qualified Name:     me.lyft.android.infrastructure.deferred.PersistentQueueManager
